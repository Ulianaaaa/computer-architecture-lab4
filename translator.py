...
class Parser:
    ...
    def consume(self, k):
        ...
        if kind == k:
            self.pos += 1
            return val
        raise RuntimeError(f"Expected {k}, got {kind!r} ({val!r}) at pos {self.pos}")
    ...
    def parse_factor(self):
        ...
        if k == "LPAREN":
            ...
            self.consume("RPAREN")
            return n
        raise RuntimeError(f"Unexpected token in factor: {k!r} ({v!r})")

    def parse_term(self):
        n = self.parse_factor()
        while self.peek()[0] == "OP" and self.peek()[1] in "*/":
            op = self.consume("OP")
            n = Node("BIN_OP", value=op, left=n, right=self.parse_factor())
        return n
    ...
    def parse_expr(self):
        ...
        while self.peek()[0] == "OP" and self.peek()[1] in "+-":
            op = self.consume("OP")
            n = Node("BIN_OP", value=op, left=n, right=self.parse_term())
        return n
    ...
    def parse_stmt(self):
        ...
        if k == "IF":
            ...
            if self.peek()[0] == "ELSE":
                ...
                self.consume("RBRACE")
            return Node("IF", value=op, children=[left, right] + body, left=Node("ELSE_BODY", children=else_body))
        ...
        if k == "FOR":
            ...
            while self.peek()[0] != "RBRACE":
                body.append(self.parse_stmt())
            self.consume("RBRACE")
            return Node("FOR", value=var_name, children=[start, end, step] + body)
...
class CodeGen:
    ...
    def gen_expr(self, n):
        ...
        if n.kind == "VAR":
            r = self.regs + 1
            self.regs += 1
            self.emit(Opcode.LW, rd=r, rs1=0, imm=self.get_addr(n.value))
            return r
        ...
        if n.kind == "STRING":
            str_name = f"str_{n.value}"
            addr = self.get_addr(str_name, len(n.value) + 1)
        ...
        if n.kind == "BIN_OP":
            ...
            r_e = self.gen_expr(n.right)
            if n.value == "+":
                self.emit(Opcode.ADD, rd=l, rs1=l, imm=r_e)
            elif n.value == "-":
                self.emit(Opcode.SUB, rd=l, rs1=l, imm=r_e)
            elif n.value == "*":
                self.emit(Opcode.MUL, rd=l, rs1=l, imm=r_e)
            elif n.value == "/":
                self.emit(Opcode.DIV, rd=l, rs1=l, imm=r_e)
            self.regs -= 1
            return l

        raise RuntimeError(f"Unknown expr node: {n.kind}")
    ...
    def gen_stmt(self, n):
        if n.kind == "ASSIGN":
            ...
            self.emit(Opcode.SW, rs1=r, rd=0, imm=self.get_addr(n.value))
            self.regs = 0

        elif n.kind == "PRINT":
            is_str = (n.left.kind == "VAR" and n.left.value in self.string_vars) or (
                n.left.kind == "STRING"
            )
            r = self.gen_expr(n.left)
            self.emit(Opcode.OUT, rs1=r, imm=0x85 if is_str else 0x84)
            self.regs = 0
            ...
            exit_j = self.emit(Opcode.BEQ, imm=0)
            self.regs = 0
            for i in range(2, len(n.children)):
                self.gen_stmt(n.children[i])
            # FIX 8: else body
            if n.left and n.left.children:
                skip_else_j = self.emit(Opcode.JMP, imm=0)
                self.instrs[exit_j]["imm"] = len(self.instrs)
                for stmt in n.left.children:
                    self.gen_stmt(stmt)
                self.instrs[skip_else_j]["imm"] = len(self.instrs)
            else:
                self.instrs[exit_j]["imm"] = len(self.instrs)
            ...
            start_lbl = len(self.instrs)
            r1 = self.gen_expr(Node("VAR", value=n.value))
            r2 = self.gen_expr(n.children[1])
            self.emit(Opcode.CMP, rs1=r1, imm=r2)
            exit_j = self.emit(Opcode.BGT, imm=0)
            self.regs = 0
            ...
            self.regs = 0
            self.emit(Opcode.JMP, imm=start_lbl)
            self.instrs[exit_j]["imm"] = len(self.instrs)
...
def main(src_f, tgt_f):
    ...
    for mo in re.finditer(regex, code):
        if mo.lastgroup not in ("SKIP", "MISMATCH"):
    ...
    for instr in gen.instrs:
        res_bin.append(OPCODE_TO_CODE[instr["opcode"]])
        res_bin.append((instr["rd"] << 4) | instr["rs1"])
        imm = instr["imm"]
        res_bin.extend([(imm >> 8) & 0xFF, imm & 0xFF])
