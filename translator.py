    ...
    def parse_stmt(self):
        ...
        if k == 'WHILE':
            ...
            while self.peek()[0] != 'RBRACE':
                body.append(self.parse_stmt())
            self.consume('RBRACE')
            return Node('WHILE', value=op, children=[l, r] + body)
        if k == 'FOR':
            self.consume('FOR')
            self.consume('LPAREN')
            v_n = self.consume('ID')
...
class CodeGen:
    def __init__(self):
        self.instrs, self.vars, self.mem, self.next_addr, self.regs, self.string_vars = [], {}, [0] * 1024, 100, 0, set()
    ...
    def gen_stmt(self, n):
        if n.kind == 'ASSIGN':
            ...
            if n.children[3].children:
                skip_else = self.emit(Opcode.JMP, imm=0)
                self.instrs[exit_if]['imm'] = len(self.instrs)
                for s in n.children[3].children:
                    self.gen_stmt(s)
            ...
            r2 = self.gen_expr(n.children[1])
            self.emit(Opcode.CMP, rs1=r1, imm=r2)
            exit_w = self.emit(Opcode.BEQ, imm=0)
            self.regs = 0
            for i in range(2, len(n.children)):
                self.gen_stmt(n.children[i])
            self.emit(Opcode.JMP, imm=start)
            self.instrs[exit_w]['imm'] = len(self.instrs)
        elif n.kind == 'FOR':
